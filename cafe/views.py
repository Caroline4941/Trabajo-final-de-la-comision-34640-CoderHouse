...
def obtenerAvatar(request):
    ...
    if len(lista)!=0:
        ...
        imagen="/media/avatares/avatar.png"
    return imagen
...
def profile(request):
    ...
    if profile:
        ...
        descripcion = ""
        link = ""
    return render(request, "cafe/profile.html", {"descripcion" : descripcion, "link": link, "imagen":obtenerAvatar(request)})
...
def editarProfile(request):
    ...
    if datos_viejos:
        formulario_edit = UserEditForm(initial={"description": datos_viejos[0].descripcion, "link": datos_viejos[0].link})
    else:
    ...
    if request.method == "POST":
        ...
        if formulario.is_valid():
            ...
            datos_nuevos = Perfil(descripcion = request.POST["description"], link = request.POST["link"])
            datos_nuevos.save()
...
def blog(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            blog.save()
            return render (request, "cafe/blog.html", {"mensaje": "BLOG CREADO", "imagen":obtenerAvatar(request)})
    else:
        formulario=BlogForm()


    return render (request, "cafe/blog.html", {"form":formulario, "imagen":obtenerAvatar(request)})
...
def agregarAvatar(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            avatar.save()
            return render(request,"cafe/inicio.html", {"mensaje":"Avatar agregado correctamente", "imagen":obtenerAvatar(request)})
        else:
            return render(request, "cafe/agregarAvatar.html", {"formulario": form, "usuario":request.user})
    else:
        form=AvatarForm()
        return render(request, "cafe/agregarAvatar.html", {"formulario": form, "usuario":request.user, "imagen":obtenerAvatar(request)})

def pages(request):
      pages=Blog.objects.all()
      print(pages)
      return render(request, "cafe/pages.html", {"blog":pages, "imagen":obtenerAvatar(request)})
